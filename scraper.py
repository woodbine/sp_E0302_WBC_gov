...
# Set up variables
entity_id = "E0302_WBC_gov"
url =  "http://info.westberks.gov.uk/index.aspx?articleid=28367"
errors = 0
# Set up functions
...
def validateURL(url):
    try:
        ...
        while response.getcode() == 500 and count < 4:
            ...
            opener = urllib2.build_opener()
            r = urllib2.Request(url)
        sourceFilename = response.headers.get('Content-Disposition')
        if sourceFilename:
...
for link in links:
    if '20' in link.text:
        ...
        csvMth = link.text.split(' ')[0].strip()[:3]
        csvYr = link.text.split(' ')[1].strip()
